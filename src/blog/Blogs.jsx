...
export const Blogs = () => {
  ...
  const { blogs, loggedIn, checkLoginState, getProfileBlog,setGetProfileBlog } = useContext(AuthContext);
            ...
            <Container className='p-4'>
              <Card>
                <Card.Body>
                  ...
                  {contentToShow(blog)}
                  {blog.Content.length <= limit ? (<Card.Text>
                    {blog.Content}
                  </Card.Text>) :
                    <Card.Text>
                      {toShow}
                    </Card.Text>}
                  <Button variant="primary" onClick={() => showMore(blog)}>View Blog</Button>
                </Card.Body>
              </Card>
            </Container>
          </div>
